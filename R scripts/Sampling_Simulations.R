...
# Simulation code #
#-----------------#
wp.sampling <- function(data.path = NULL, population = c("TPSR","SRI"), sheet = 2, 
                        trait = NULL, rep = 500, digits = 1, out.path = NULL){

  #-- Loading data, standardization, select population, and remove missing values --#
  data <- readxl::read_excel(data.path, sheet = sheet); data <- data.frame(data)
  ...
  if(population=="TPSR" & length(fam)!=80){stop(paste("Number of maternal families set to: ", 1, ":", nb.fam, "\n\n", sep = ""))}
  if(population=="TPSR" & length(fam)==80){cat(paste("Range of maternal families tested: ", 1, ":", nb.fam, "\n\n", sep = ""))}
  if(population=="SRI" & length(fam)!=30){stop(paste("Number of maternal families set to: ", 1, ":", nb.fam, "\n\n", sep = ""))}
  ...
  cat("\n")
  setwd(out.path)
  xlsx::write.xlsx(out, file = paste(population,"_", names(data)[trait],"_it",rep, 
...
for(j in 1:1){
  for(l in 3:16){
    wp.sampling(data.path = "./Data/Results_Spreadsheet_5seedPerMF.xlsx", population = "TPSR", sheet = "Derivable",
...
for(j in 1:1){
  for(l in 3:16){
    wp.sampling(data.path = "./Data/Results_Spreadsheet_5seedPerMF.xlsx", population = "SRI", sheet = "Derivable",
                trait = l, rep = 500, digits = j, out.path = "./Simulation_Outputs/")
    cat("\n\n")
  }
}