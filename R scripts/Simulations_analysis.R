...
for(i in 1:length(mainland)){
  ...
  mainland[[i]] <- mainland[[i]][,-1] # Remove column with # of iterations
...
for(i in 1:length(island)){
  ...
  island[[i]] <- island[[i]][,-1] # Remove column with # of iterations
...
d1c <- ggplot()+
  geom_point(data=out_m, aes(x = nbfam/80, y= mean), colour="orange")+
  geom_line(data=out_m, aes(x = nbfam/80, y= mean), colour="orange")+
  geom_point(data = out_i, aes(x = nbfam/30, y= mean), colour="forestgreen")+
  geom_line(data = out_i, aes(x = nbfam/30, y= mean), colour="forestgreen")+
  #geom_errorbar(data = out_m, aes(x = nbfam/80, ymin = mean-se, ymax = mean+se, width = 0.015), colour="orange")+
  #geom_errorbar(data = out_i, aes(x = nbfam/30, ymin = mean-se, ymax = mean+se, width = 0.015), colour="forestgreen")+
  theme_classic()+xlab("\nProportion of maternal families sampled")+
  ylab(expression(paste("N"["c"],"/N"["t"])))+
  geom_vline(xintercept = 57/80, linetype=4, colour = "orange")+
  geom_vline(xintercept = 25/30, linetype=4, colour = "forestgreen")+
