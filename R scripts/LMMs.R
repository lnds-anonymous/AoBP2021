#-------------------------------------------#
# LM models for Torrey pine seed morphology #
#-------------------------------------------#
...
setwd("../Data")
df <- readxl::read_excel("Results_Spreadsheet_5seedPerMF.xlsx", sheet = "Derivable"); df <- data.frame(df)
...
resmelt$Var1 <- factor(resmelt$Var1, levels = c("RSCS", "REndS", "RES", "SCA", "ELW","SLW", "EA",
                                                "ESA", "SA", "EW", "EL", "SCW", "SW", "SL"))
...
ggplot(data=resmelt, aes(x=Var1, y=value, fill=Var2))+geom_bar(stat = "identity", position = position_stack(reverse = T))+
  ...
  scale_fill_manual(labels = c("Populations", "Maternal families", "Residuals"),
                    values = c("forestgreen", "orange", "red4"),
                    name = element_blank())+
