...
# 2) Do differences in seed traits impact emergence? (Linear regression and t-tests)
...
ggbiplot(dudi, scale=0, groups = data$Source, ellipse=T, var.axes = F)+
  ...
  scale_color_manual(values = c("forestgreen", "orange"), labels=c(" Island", " Mainland"), name = "Population")+theme_bw()+
...
ggplot()+geom_boxplot(data=df, aes(x=Date, y=Perc_complete, fill=Source))+#coord_flip()+
  scale_fill_manual(values = c("forestgreen", "orange"), labels=c(" Island", " Mainland"))+
